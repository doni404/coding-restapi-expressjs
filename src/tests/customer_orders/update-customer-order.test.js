...
const testKey = "update-customer-order";

describe('/customer-order update data put and patch', () => {
    let createdAdmin;
    let createdProduct;
    let createdCustomer;
    ...
    it('should update customer order with the valid data', async () => {
        const response = await request(app)
            .put('/v1/cms/customer-orders/' + createdCustomerOrder.id)
            .set('Authorization', `Bearer ${jwtToken}`)
            .send({
                customer_store_id: createdCustomerStore.id,
                price: 1000000,
                tax: 10000,
                situation: "process",
                store_name: "updated" + createdCustomerStore.name,
                store_zip: "updated" + createdCustomerStore.zip,
                store_prefecture: "updated" + createdCustomerStore.prefecture,
                store_address: "updated" + createdCustomerStore.address,
                payment_method: "transfer",
                bank_name: "Bank Mandiri",
                account_name: "William Daendals Updated",
                account_number: "1420022248958",
                note: "note",
                items: [
                    {
    ...
    it('should return an error with no authorization header (update customer orders)', async () => {
        const response = await request(app)
            .put('/v1/cms/customer-orders/' + createdCustomerOrder.id)
            .send({
                customer_store_id: createdCustomerStore.id,
    ...
    it('should update customer order send date with the valid data', async () => {
        const response = await request(app)
            .patch('/v1/cms/customer-orders/' + createdCustomerOrder.id + '/send-date')
            .set('Authorization', `Bearer ${jwtToken}`)
            .send({
                send_date: "2023-10-13 23:08:06"
            });
    ...
    it('should update customer order delivered date with the valid data', async () => {
        const response = await request(app)
            .patch('/v1/cms/customer-orders/' + createdCustomerOrder.id + '/delivered-date')
            .set('Authorization', `Bearer ${jwtToken}`)
            .send({
                delivered_date: "2023-10-13 23:08:06"
            });
    ...
    it('should update customer order reconcile price with the valid data', async () => {
        const response = await request(app)
            .patch('/v1/cms/customer-orders/' + createdCustomerOrder.id + '/reconcile')
            .set('Authorization', `Bearer ${jwtToken}`)
            .send({
                price_reconciled: 150000,
                tax_reconciled: 1500
            });
