...
describe('/customer-order create endpoint', () => {
    let createdAdmin;
    let createdProduct;
    let createdCustomer;
    ...
    it('should create a customer order with the valid data', async () => {
        const response = await request(app)
        ...
        createdCustomerOrder = response.body.data;
    });

    it('should return an error with an empty items', async () => {
        const response = await request(app)
            .post('/v1/cms/customer-orders')
            .set('Authorization', `Bearer ${jwtToken}`)
            .send({
